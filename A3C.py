...
class Worker():
	def __init__(self, arena, name, model_path, global_episodes, agent_host, recordingsDirectory, trainer, gamma=0.99):
		...
		self.rewards = {"health": -1, "kills": 8.0, "ontarget": 0.5, "inrange": 1, "movement": -.01, "miss": -.2}
			...
			while not coord.should_stop():
				...
				for retry in range(max_retries):
					try:
						...
						if self.number == 1:
							pool.add(MalmoPython.ClientInfo('10.207.161.221', 10000))
						if self.number == 2:
							pool.add(MalmoPython.ClientInfo('10.207.161.221', 10001))
						if self.number == 3:
							pool.add(MalmoPython.ClientInfo('10.207.161.221', 10002))
						if self.number == 4:
							pool.add(MalmoPython.ClientInfo('127.0.0.1', 10001))
						self.agent_host.startMission(my_mission, pool, self.my_mission_record, 0, experimentID)
						break
				...
				# self.agent_host.sendCommand("chat /gamerule keepInventory false")
				self.agent_host.sendCommand("chat /difficulty easy")
				# self.agent_host.sendCommand("chat /experience set @p 0")
				# Wait for everything to load
				# self.arena.killspawns()
				# time.sleep(0.3)
				...
				while (world_state.is_mission_running):
					...
					runnning = world_state.is_mission_running
					if runnning:
						try:
							world_state = self.agent_host.getWorldState()
							frame = world_state.video_frames[0]
						...
						s1 = s
					episode_buffer.append([s, a, r, s1, runnning, v[0, 0]])
					...
					if len(episode_buffer) == 30 and runnning and episode_step_count != max_episode_length - 1:
				...
				if episode_count % 5 == 0 and episode_count != 0:
					if self.name == 'worker_0' and episode_count % 25 == 0:
						...
						episode_frames[0].save('./frames/image'+str(episode_count)+'.gif', format='GIF', append_images=episode_frames[1:], save_all=True, duration=len(episode_frames)*time_per_step, loop=0)
				...
				print('Worker number: ', str(self.name))
